...
class QuantileNet(nn.Module):
    def __init__(
        self, input_dim: int, hidden_dims: list = [32, 32], use_residual: bool = False
    ):
        super().__init__()
        """
        初始化NC-CQR网络
        ...
        self.median_out = nn.Linear(prev_dim, 1)  # τ2=0.5 (中位数)
        self.upper_out = nn.Linear(prev_dim, 1)  # τ3=0.975 (上分位数)

    def forward(self, x):
        """前向传播"""
        shared = self.shared_layers(x)
        return self.lower_out(shared), self.median_out(shared), self.upper_out(shared)


def quantile_loss(
    output: torch.Tensor, target: torch.Tensor, tau: float
) -> torch.Tensor:
...
def three_quantile_non_crossing_loss(
    ...
    tau_high: float = 0.975,
    lambda_penalty: float = 1.0,
) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
    """三分位数非交叉损失函数"""
    # 各分位数的pinball损失
    loss_lower = quantile_loss(lower_pred, target, tau_low)
    loss_median = quantile_loss(median_pred, target, tau_median)
    loss_upper = quantile_loss(upper_pred, target, tau_high)
    
    # 非交叉约束：确保 lower ≤ median ≤ upper
    crossing_penalty_1 = torch.mean(torch.relu(lower_pred - median_pred))
    crossing_penalty_2 = torch.mean(torch.relu(median_pred - upper_pred))
    total_crossing_penalty = crossing_penalty_1 + crossing_penalty_2
    
    total_loss = loss_lower + loss_median + loss_upper + lambda_penalty * total_crossing_penalty
    return total_loss, loss_lower, loss_median, loss_upper, total_crossing_penalty


def non_crossing_quantile_loss(
    lower_pred: torch.Tensor,
    upper_pred: torch.Tensor,
    target: torch.Tensor,
    tau_low: float = 0.025,
...
def train_nc_cqr_model(
    ...
    X_calib: np.ndarray,
    y_calib: np.ndarray,
    epochs: int = 200,          # 优化：从150增加到200轮
    batch_size: int = 64,       # 优化：从32增加到64
    learning_rate: float = 2e-3, # 优化：从4e-3降低到2e-3
    hidden_dims: list = [64, 64], # 优化：从[32,32]扩大到[64,64]
    ...
    if len(X_train) < batch_size:
        print(
            f"警告：训练数据量({len(X_train)})少于批次大小({batch_size})，自动调整批次大小为{len(X_train)}"
        )
    ...
    # 初始化模型（支持残差块）
    model = QuantileNet(
        input_dim=X_train.shape[1], hidden_dims=hidden_dims, use_residual=use_residual
    ).to(device)
    ...
    for epoch in range(epochs):
        ...
        for X_batch, y_batch in train_loader:
            ...
            lower_pred, median_pred, upper_pred = model(X_batch)

            total_loss, loss_lower, loss_median, loss_upper, crossing_penalty = (
                three_quantile_non_crossing_loss(
                    lower_pred,
                    median_pred,
                    upper_pred,
                    y_batch,
            ...
            epoch_median_loss += loss_median.item()
        ...
        if (epoch + 1) % 50 == 0 or epoch == 0 or (epoch + 1) == epochs:
            ...
            avg_median = epoch_median_loss / num_batches
            ...
            print(f"  中位数损失: {avg_median:.6f}")
            print(f"  上分位数损失: {avg_upper:.6f}")
            print(
                f"  交叉惩罚: {avg_crossing:.6f} {'[警告]' if avg_crossing > 0.01 else '[正常]'}"
            )
            ...
            if avg_crossing > 0.2:
                lambda_penalty *= 1.5
                print(
                    f"  [警告] 交叉惩罚过高，增加lambda_penalty到{lambda_penalty:.1f}"
                )
    ...
    with torch.no_grad():
        ...
        lower_pred, median_pred, upper_pred = model(X_calib_tensor)
        conformity_scores = torch.maximum(
            lower_pred.squeeze() - y_calib_tensor, y_calib_tensor - upper_pred.squeeze()
        )
        ...
        if violation_rate <= alpha + 0.05:
            ...
            print(
                f"[警告] Conformal校准可能有问题，目标违约率{alpha:.1%}，实际违约率{violation_rate:.1%}"
            )
...
def evaluate_model(
    model: nn.Module, X_test: np.ndarray, y_test: np.ndarray, Q: float
) -> Dict:
    """评估NC-CQR模型"""
    ...
    with torch.no_grad():
        ...
        lower_pred, median_pred, upper_pred = model(X_test_tensor)
...
def load_model(model_path: str) -> Tuple[nn.Module, float, Dict]:
    ...
    if "median_out.weight" not in state_dict:
        print("检测到旧版本模型（无中位数输出），进行兼容性处理...")
        # 为中位数输出头初始化权重（使用上下分位数的均值）
        if "lower_out.weight" in state_dict and "upper_out.weight" in state_dict:
            median_weight = (state_dict["lower_out.weight"] + state_dict["upper_out.weight"]) / 2
            median_bias = (state_dict["lower_out.bias"] + state_dict["upper_out.bias"]) / 2
            state_dict["median_out.weight"] = median_weight
            state_dict["median_out.bias"] = median_bias
...
def train_full_pipeline(
    ...
    **train_kwargs,
    ...
    # 确保使用城市特定的确定性种子（关键修复）
    from .reproducibility import get_city_seed, set_deterministic_seeds
    city_seed = get_city_seed(city)
    ...
    if abs(train_ratio + calib_ratio + test_ratio - 1.0) > 1e-6:
        raise ValueError(
            f"数据集比例之和必须为1.0，当前为{train_ratio + calib_ratio + test_ratio}"
        )
    ...
    model, Q = train_nc_cqr_model(
        X_train, y_train, X_calib, y_calib, **train_kwargs_with_city
    )
