#!/bin/bash
# 阿里云ECS服务配置脚本 - 修复版
# 自动检测用户配置Nginx、Gunicorn、systemd服务
...
echo -e "${BLUE}===========================================${NC}"
echo -e "${BLUE}    配置Web服务和应用服务 (修复版)${NC}"
...
echo -e ""
echo -e "${YELLOW}如果服务启动失败，可以尝试:${NC}"
echo -e "  sudo systemctl restart no2-prediction"
echo -e "  sudo systemctl reload nginx"

# 保存配置信息
echo -e "${BLUE}配置信息已保存到 /tmp/no2_deploy_info.txt${NC}"
cat > /tmp/no2_deploy_info.txt << EOF
NO2预测系统部署信息
==================
部署时间: $(date)
服务器IP: $SERVER_IP
